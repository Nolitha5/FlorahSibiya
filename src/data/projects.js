export const projects = [
  {
    id: 'personal-os',
    title: 'PersonalOS — AI Productivity Platform',
    tag: 'AI · Automation · Featured',
    description:
      'An AI-powered personal productivity system built with Claude Code. Integrates three automation modules: Sprint Tracker (Notion-based), Morning Brief (Gmail + Calendar summary), and Market Pulse (intelligent trend monitoring). Notion acts as the central data layer; deployed via Vercel with continuous integration.',
    image: '/assets/personalOS.jpeg',
    video: '/assets/PersonalOS3in1.mp4',
    docs: '/assets/personal_os_documentation.pdf',
    liveDemo: 'https://personal-os-git-main-florah-s-projects.vercel.app/',
    colab: null,
    featured: true,
    techs: ['Claude API', 'React', 'Node.js', 'Notion API', 'Gmail API', 'Vercel'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5/PersonalOS', type: 'github' },
    ],
  },
  {
    id: 'rew-agent',
    title: 'Academic Risk Early Warning Agent',
    tag: 'Multi-Agent AI · EdTech · POPIA-Compliant',
    description:
      'A multi-agent agentic system that continuously monitors student academic signals across seven specialist domains — performance, attendance, psychosocial risk, module difficulty, temporal trends, and intervention history — synthesising evidence into a risk-stratified report delivered to human academic advisors before students reach the point of no return.',
    image: '/assets/rewpp.png',
    video: null,
    docs: '/assets/rew-agent-documentation.pdf',
    liveDemo: 'https://academic-risk-early-warning-agent.vercel.app/',
    colab: null,
    featured: false,
    techs: ['Python', 'Multi-Agent AI', 'RAG', 'FastAPI', 'React', 'Supabase'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5', type: 'github' },
      { label: 'Architecture', href: '/assets/rew-architecture.jpg', type: 'architecture' },
    ],
  },
  {
    id: 'demand-sensing',
    title: 'Demand Sensing Agent',
    tag: 'AI · Time-Series · Forecasting',
    description:
      'An AI-powered demand forecasting agent that analyses historical sales data to predict future demand using time-series modelling. Evaluates model accuracy with MAE, WAPE, and Forecast Bias metrics, and surfaces actionable inventory recommendations through a React dashboard backed by a FastAPI service.',
    image: '/assets/DemandPic.png',
    video: null,
    docs: null,
    liveDemo: null,
    colab: null,
    featured: false,
    techs: ['Python', 'Pandas', 'NumPy', 'Time-Series', 'FastAPI', 'React', 'MAE / WAPE'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5', type: 'github' },
    ],
  },
  {
    id: 'career-guidance-agent',
    title: 'Career Guidance Agent — AI Career Recommendation System',
    tag: 'AI · Full-Stack · React',
    description:
      'A faculty-aware career recommendation system built with React 18 + Vite and Supabase. Guides university students across 7 faculties and 28+ career paths through a 6-question quiz, scoring engine powered by a 5,000-record Kaggle dataset, and auto-saved PDF reports — with SA-localised salary ranges and role-specific job-readiness steps.',
    image: '/assets/careerguidanceagentt.jpg',
    video: null,
    docs: '/assets/career_guidance_agent_documentation.pdf',
    liveDemo: 'https://career-guidance-agentt-two.vercel.app/',
    colab: null,
    featured: false,
    techs: ['React', 'Vite', 'Supabase', 'Python', 'Kaggle Dataset', 'PDF Generation'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5/CareerGuidanceAgentt', type: 'github' },
    ],
  },
  {
    id: 'wil-placement-agent',
    title: 'WIL Placement Agent',
    tag: 'AI · Full-Stack · Portal',
    description:
      'An intelligent WIL (Work Integrated Learning) placement agent that streamlines the placement process for students and institutions — matching students to opportunities, tracking applications, and automating administrative workflows.',
    image: '/assets/wil.png',
    video: null,
    docs: '/assets/wil_placement_agent_documentation.pdf',
    liveDemo: 'https://wil-placement-agent.vercel.app/',
    colab: null,
    featured: false,
    techs: ['React', 'Node.js', 'Supabase', 'AI Agents', 'REST APIs'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5', type: 'github' },
    ],
  },
  {
    id: 'career-guidance',
    title: 'Smart Career Guidance Assistant',
    tag: 'Multi-Agent AI',
    description:
      'Multi-agent AI system built in Google Colab with AutoGen. Analyses education, skills, and aspirations to generate career path recommendations, skills gap analysis, and real-time labour market assessments through coordinated specialist agents.',
    image: '/assets/Career.png',
    video: '/assets/SmartCareerguidance.mp4',
    docs: 'https://colab.research.google.com/drive/1ThZYO6gBBGuM-E4Tlc1vn4io_dF55zF-',
    liveDemo: null,
    colab: 'https://colab.research.google.com/drive/1ThZYO6gBBGuM-E4Tlc1vn4io_dF55zF-',
    featured: false,
    techs: ['AutoGen', 'Python', 'Google Colab', 'Multi-Agent AI', 'LLM Integration'],
    links: [],
  },
  {
    id: 'clothing-intelligence',
    title: 'AI Clothing Store Competitor Intelligence',
    tag: 'LangGraph · Intelligence',
    description:
      'LangGraph-based agentic system that analyses local clothing retail competition. A graph-based multi-agent workflow handles data extraction, pricing analysis, and brand positioning to surface actionable market opportunities — powered by GPT-4 Turbo and real-time web search via Tavily.',
    image: '/assets/ClothingAnalysis.webp',
    video: null,
    docs: 'https://colab.research.google.com/drive/1ThZYO6gBBGuM-E4Tlc1vn4io_dF55zF-',
    liveDemo: null,
    colab: 'https://colab.research.google.com/drive/1ThZYO6gBBGuM-E4Tlc1vn4io_dF55zF-',
    featured: false,
    techs: ['LangGraph', 'GPT-4 Turbo', 'Tavily', 'Python', 'Streamlit'],
    links: [],
  },
  {
    id: 'health-assistant',
    title: 'Smart Health Assistant',
    tag: 'Agentic AI · Health',
    description:
      'Agentic AI wellness tool that calculates BMI from user inputs and autonomously generates personalised exercise and nutrition recommendations using AutoGen agent coordination. Built with Gradio for an accessible browser interface.',
    image: '/assets/bmipic.png',
    video: '/assets/SHA.mp4',
    docs: 'https://colab.research.google.com/drive/1ThZYO6gBBGuM-E4Tlc1vn4io_dF55zF-',
    liveDemo: null,
    colab: 'https://colab.research.google.com/drive/1ThZYO6gBBGuM-E4Tlc1vn4io_dF55zF-',
    featured: false,
    techs: ['AutoGen', 'Python', 'Gradio', 'AI Agents', 'Google Colab'],
    links: [],
  },
  {
    id: 'smarttech',
    title: 'SmartTech — CV Scanner & Job Portal',
    tag: 'AI · NLP · Job Portal',
    description:
      'Three-phase platform: company website → AI job portal → NLP-powered CV scanner that automatically matches candidates to job requirements with a real-time tracking dashboard.',
    image: '/assets/admin_log.jpg',
    video: null,
    docs: 'https://github.com/Nolitha5/SmartTech-Web/blob/main/README.md',
    liveDemo: null,
    colab: null,
    featured: false,
    techs: ['Python', 'NLP', 'React', 'Django', 'REST APIs'],
    links: [{ label: 'GitHub', href: 'https://github.com/Nolitha5/SmartTech-Web', type: 'github' }],
  },
  {
    id: 'doctor-appointment',
    title: 'Doctor Appointment Booking System',
    tag: 'Full-Stack · Healthcare',
    description:
      'A web application enabling patients to book, manage, and track medical appointments online. Centralised platform for scheduling and doctor-patient communication.',
    image: '/assets/projImage.png',
    video: null,
    docs: 'https://github.com/Nolitha5/DoctorsAppointmentBooking/blob/main/README.md',
    liveDemo: 'https://dr-appointment-6316.vercel.app/',
    colab: null,
    featured: false,
    techs: ['React', 'Node.js', 'MySQL', 'REST APIs', 'Vercel'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5/DoctorsAppointmentBooking/', type: 'github' },
    ],
  },
  {
    id: 'ecommerce',
    title: 'E-Commerce Web Application',
    tag: 'E-Commerce · Full-Stack',
    description:
      'Full-featured online store where users can browse and purchase products, with built-in price comparison across categories.',
    image: '/assets/storr.png',
    video: null,
    docs: 'https://github.com/Nolitha5/ecom/blob/main/README.md',
    liveDemo: 'https://ecom-r26z.onrender.com/',
    colab: null,
    featured: false,
    techs: ['Django', 'Python', 'JavaScript', 'PostgreSQL', 'Render'],
    links: [
      { label: 'GitHub', href: 'https://github.com/Nolitha5/ecom', type: 'github' },
    ],
  },
  {
    id: 'fingerprint-scanner',
    title: 'Fingerprint Access Scanner System',
    tag: 'Biometric · Security',
    description:
      'Biometric security solution replacing key/password access with fingerprint authentication. Controls and monitors entry to restricted areas with a fast, user-friendly identification flow.',
    image: '/assets/fingerprint.png',
    video: null,
    docs: 'https://github.com/Nolitha5/fingerprint/blob/main/README.md',
    liveDemo: null,
    colab: null,
    featured: false,
    techs: ['Python', 'Biometrics', 'Hardware Integration', 'Security'],
    links: [{ label: 'GitHub', href: 'https://github.com/Nolitha5/fingerprint', type: 'github' }],
  },
  {
    id: '4ir-application',
    title: '4IR Online Application System',
    tag: 'Web App · 4IR',
    description:
      'Student application portal for 4IR-TUT programmes. Handles personal details, specialisation selection, document uploads, and form submissions in a streamlined flow.',
    image: '/assets/4ir.jpg',
    video: null,
    docs: 'https://github.com/Nolitha5/Form/blob/main/README.md',
    liveDemo: null,
    colab: null,
    featured: false,
    techs: ['React', 'JavaScript', 'HTML', 'CSS', 'Form Handling'],
    links: [{ label: 'GitHub', href: 'https://github.com/Nolitha5/Form', type: 'github' }],
  },
]
